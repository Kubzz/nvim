return {
    {
        ...
        lazy = false,
        keys = {
            { "<leader>fe", function() require("neo-tree.command").execute({ toggle = true, dir = vim.uv.cwd() }) end, desc = "Explorer NeoTree (Root Dir) - Current doesn CWD / Broken" },
            { "<leader>fE", function() require("neo-tree.command").execute({ toggle = true, dir = vim.uv.cwd() }) end, desc = "Explorer NeoTree (cwd)", },
            { "<leader>e", "<leader>fe", desc = "Explorer NeoTree (Root Dir) - Current does CWD / Broken", remap = true },
